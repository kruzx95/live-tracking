 ...
 * @param {number} points
 */
export function generateDemoRoute(center = { lat: -7.2575, lon: 112.7521 }, radiusKm = 5, points = 200) {
  const trackPoints = [];
  ...
  for (let i = 0; i <= points; i++) {
    ...
    const lat = center.lat + r * Math.sin(angle) * 0.8;
    const lon = center.lon + r * Math.cos(angle);
    const ele = 50 + 30 * Math.sin(angle * 3); // simulasi elevasi
    trackPoints.push({ lat, lon, ele, time: null });
  }

  const waypoints = [
    { lat: center.lat + r, lon: center.lon, name: 'Start / Finish', type: 'start' },
    { lat: center.lat, lon: center.lon + r, name: 'CP 1 — Indomaret', type: 'store' },
    { lat: center.lat - r * 0.8, lon: center.lon, name: 'CP 2 — Water Station', type: 'water' },
  ];

  return {
    name: 'Demo Route — Loop 10km',
    trackPoints,
    waypoints,
    stats: {
      totalDistance: parseFloat((2 * Math.PI * radiusKm).toFixed(2)),
      totalElevGain: 180,
      totalElevLoss: 178,
      minEle: 30,
      maxEle: 88,
    ...
    elevationProfile: trackPoints
      .filter((_, i) => i % 4 === 0)
      .map((pt, i) => ({ distance: parseFloat((i * 0.2).toFixed(1)), ele: Math.round(pt.ele) })),
    bounds: [
      [center.lat - r, center.lon - r],
      [center.lat + r, center.lon + r],
    ],
