/**
 * gpxParser.js
 * Utility untuk membaca dan mem-parsing file .gpx
 * Mengekstrak: koordinat (lat/lon), elevasi, nama waypoints/checkpoint
 * Menghitung: total jarak, total elevation gain/loss
 */

/**
 * Haversine formula — menghitung jarak antara dua koordinat GPS (km)
 */
export function haversineDistance(lat1, lon1, lat2, lon2) {
  const R = 6371; // Radius bumi km
  const dLat = ((lat2 - lat1) * Math.PI) / 180;
  const dLon = ((lon2 - lon1) * Math.PI) / 180;
  const a =
    Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.cos((lat1 * Math.PI) / 180) *
      Math.cos((lat2 * Math.PI) / 180) *
      Math.sin(dLon / 2) *
      Math.sin(dLon / 2);
  return R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

/**
 * Parse file .gpx dan kembalikan data rute terstruktur
 * @param {File|string} input - File object atau XML string
 * @returns {Promise<RouteData>}
 */
export async function parseGPX(input) {
  let xmlString;

  if (input instanceof File) {
    xmlString = await input.text();
  } else {
    xmlString = input;
  }

  const parser = new DOMParser();
  const doc = parser.parseFromString(xmlString, 'application/xml');

  const parseError = doc.querySelector('parsererror');
  if (parseError) {
    throw new Error('File GPX tidak valid atau rusak.');
  }

  // Ambil nama rute
  const routeName =
    doc.querySelector('metadata > name')?.textContent ||
    doc.querySelector('trk > name')?.textContent ||
    'Rute Event';

  // Ekstrak track points dari <trkpt> atau <rtept>
  const trackPoints = [];
  const trkpts = doc.querySelectorAll('trkpt, rtept');

  trkpts.forEach((pt) => {
    const lat = parseFloat(pt.getAttribute('lat'));
    const lon = parseFloat(pt.getAttribute('lon'));
    const ele = parseFloat(pt.querySelector('ele')?.textContent || '0');
    const timeStr = pt.querySelector('time')?.textContent;
    const time = timeStr ? new Date(timeStr).getTime() : null;

    if (!isNaN(lat) && !isNaN(lon)) {
      trackPoints.push({ lat, lon, ele, time });
    }
  });

  // Ekstrak waypoints (checkpoints, water stations, dll dari <wpt>)
  const waypoints = [];
  const wpts = doc.querySelectorAll('wpt');

  wpts.forEach((wpt) => {
    const lat = parseFloat(wpt.getAttribute('lat'));
    const lon = parseFloat(wpt.getAttribute('lon'));
    const name = wpt.querySelector('name')?.textContent || 'Checkpoint';
    const desc = wpt.querySelector('desc')?.textContent || '';
    const sym  = wpt.querySelector('sym')?.textContent?.toLowerCase() || '';

    // Tentukan tipe waypoint dari nama / simbol
    let type = 'checkpoint';
    if (sym.includes('flag') || name.toLowerCase().includes('start')) type = 'start';
    else if (sym.includes('finish') || name.toLowerCase().includes('finish')) type = 'finish';
    else if (name.toLowerCase().includes('indomaret') || name.toLowerCase().includes('alfamart') || name.toLowerCase().includes('minimarket')) type = 'store';
    else if (name.toLowerCase().includes('water') || name.toLowerCase().includes('air') || name.toLowerCase().includes('ws')) type = 'water';
    else if (name.toLowerCase().includes('cp') || name.toLowerCase().includes('checkpoint')) type = 'checkpoint';

    if (!isNaN(lat) && !isNaN(lon)) {
      waypoints.push({ lat, lon, name, desc, type });
    }
  });

  // Hitung total jarak & elevasi
  let totalDistance = 0; // km
  let totalElevGain = 0; // meter
  let totalElevLoss = 0; // meter
  let minEle = Infinity;
  let maxEle = -Infinity;

  for (let i = 1; i < trackPoints.length; i++) {
    const prev = trackPoints[i - 1];
    const curr = trackPoints[i];

    totalDistance += haversineDistance(prev.lat, prev.lon, curr.lat, curr.lon);

    const dEle = curr.ele - prev.ele;
    if (dEle > 0) totalElevGain += dEle;
    else          totalElevLoss += Math.abs(dEle);

    if (curr.ele < minEle) minEle = curr.ele;
    if (curr.ele > maxEle) maxEle = curr.ele;
  }

  // Buat elevation profile (sample setiap ~50 titik agar ringan)
  const sampleRate = Math.max(1, Math.floor(trackPoints.length / 100));
  const elevationProfile = trackPoints
    .filter((_, i) => i % sampleRate === 0)
    .map((pt, i) => ({
      distance: parseFloat((i * sampleRate * (totalDistance / trackPoints.length)).toFixed(2)),
      ele: Math.round(pt.ele),
    }));

  // Bounding box untuk auto-fit peta
  const lats = trackPoints.map((p) => p.lat);
  const lons = trackPoints.map((p) => p.lon);
  const bounds = [
    [Math.min(...lats), Math.min(...lons)],
    [Math.max(...lats), Math.max(...lons)],
  ];

  // Koordinat start & finish
  const startPoint = trackPoints[0] || null;
  const finishPoint = trackPoints[trackPoints.length - 1] || null;

  return {
    name: routeName,
    trackPoints,       // Array koordinat lengkap [{lat, lon, ele, time}]
    waypoints,         // Checkpoints, water stations, dll
    stats: {
      totalDistance: parseFloat(totalDistance.toFixed(2)), // km
      totalElevGain: Math.round(totalElevGain),            // m
      totalElevLoss: Math.round(totalElevLoss),            // m
      minEle: Math.round(minEle === Infinity ? 0 : minEle),
      maxEle: Math.round(maxEle === -Infinity ? 0 : maxEle),
      pointCount: trackPoints.length,
    },
    elevationProfile,
    bounds,
    startPoint,
    finishPoint,
  };
}

/**
 * Hitung jarak rider dari start berdasarkan posisi GPS saat ini
 * dengan mencocokkan ke track point terdekat (nearest-point matching)
 * @param {object} riderPos - { lat, lon }
 * @param {Array}  trackPoints - Array dari parseGPX
 * @returns {number} Jarak tempuh estimasi (km)
 */
export function estimateDistanceTraveled(riderPos, trackPoints) {
  if (!trackPoints || trackPoints.length === 0) return 0;

  let minDist = Infinity;
  let nearestIdx = 0;

  for (let i = 0; i < trackPoints.length; i++) {
    const d = haversineDistance(riderPos.lat, riderPos.lon, trackPoints[i].lat, trackPoints[i].lon);
    if (d < minDist) {
      minDist = d;
      nearestIdx = i;
    }
  }

  // Hitung akumulasi jarak sampai nearest index
  let dist = 0;
  for (let i = 1; i <= nearestIdx; i++) {
    dist += haversineDistance(
      trackPoints[i - 1].lat, trackPoints[i - 1].lon,
      trackPoints[i].lat, trackPoints[i].lon
    );
  }

  return parseFloat(dist.toFixed(2));
}

/**
 * Cek apakah rider sedang off-course
 * @param {object} riderPos - { lat, lon }
 * @param {Array}  trackPoints
 * @param {number} thresholdKm - Batas toleransi jarak dari rute (default 0.15 km = 150m)
 * @returns {boolean}
 */
export function isOffCourse(riderPos, trackPoints, thresholdKm = 0.15) {
  if (!trackPoints || trackPoints.length === 0) return false;

  const minDist = Math.min(
    ...trackPoints.map((pt) =>
      haversineDistance(riderPos.lat, riderPos.lon, pt.lat, pt.lon)
    )
  );

  return minDist > thresholdKm;
}

/**
 * Convert trackPoints ke format LatLng array untuk Leaflet Polyline
 * @param {Array} trackPoints
 * @returns {Array} [[lat, lon], ...]
 */
export function trackToLatLngs(trackPoints) {
  return trackPoints.map((pt) => [pt.lat, pt.lon]);
}

/**
 * Demo route: Rute loop demo (tidak perlu file GPX)
 * Menghasilkan rute simulasi melingkar untuk testing
 * @param {object} center - { lat, lon }
 * @param {number} radiusKm
 * @param {number} points
 */
const REAL_ROAD_POINTS = [
  {
    "lat": -7.271524,
    "lon": 112.742975,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.271867,
    "lon": 112.742861,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.272115,
    "lon": 112.742769,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.272271,
    "lon": 112.74271,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.272444,
    "lon": 112.742644,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.27252,
    "lon": 112.742615,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.272596,
    "lon": 112.742586,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.272625,
    "lon": 112.742575,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.27276,
    "lon": 112.742524,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.272806,
    "lon": 112.742534,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.273196,
    "lon": 112.742389,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.273336,
    "lon": 112.742339,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.273557,
    "lon": 112.742252,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.273961,
    "lon": 112.742105,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.274267,
    "lon": 112.742005,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.274585,
    "lon": 112.741915,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.274995,
    "lon": 112.741815,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.275261,
    "lon": 112.741758,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.275422,
    "lon": 112.741728,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.276326,
    "lon": 112.741561,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.276591,
    "lon": 112.741511,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.276992,
    "lon": 112.741438,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.277094,
    "lon": 112.741418,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.277339,
    "lon": 112.741369,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.277411,
    "lon": 112.741355,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.27761,
    "lon": 112.741319,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.277789,
    "lon": 112.741286,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.277958,
    "lon": 112.741255,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.278113,
    "lon": 112.741228,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.278164,
    "lon": 112.741219,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.278311,
    "lon": 112.741193,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.279579,
    "lon": 112.740965,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.279665,
    "lon": 112.740949,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.280034,
    "lon": 112.740874,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.280298,
    "lon": 112.74082,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.280389,
    "lon": 112.740801,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.280981,
    "lon": 112.740699,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.281185,
    "lon": 112.740659,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.281361,
    "lon": 112.74063,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.282614,
    "lon": 112.740402,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.283248,
    "lon": 112.740281,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.284097,
    "lon": 112.740118,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.2842,
    "lon": 112.740098,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.284475,
    "lon": 112.740046,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.284513,
    "lon": 112.740038,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.28458,
    "lon": 112.740023,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.284751,
    "lon": 112.739985,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.28483,
    "lon": 112.739971,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.285802,
    "lon": 112.739795,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.285973,
    "lon": 112.739761,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.286093,
    "lon": 112.73974,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.286205,
    "lon": 112.739719,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.286256,
    "lon": 112.73971,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.287085,
    "lon": 112.739572,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.287359,
    "lon": 112.739517,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.287577,
    "lon": 112.739476,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.287721,
    "lon": 112.739447,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.287973,
    "lon": 112.739398,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.288385,
    "lon": 112.739325,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.288618,
    "lon": 112.739283,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.288745,
    "lon": 112.73926,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.288903,
    "lon": 112.739226,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.289175,
    "lon": 112.739172,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.289322,
    "lon": 112.739145,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.289333,
    "lon": 112.739143,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.289445,
    "lon": 112.73912,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.289578,
    "lon": 112.739096,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.289686,
    "lon": 112.739081,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289794,
    "lon": 112.739066,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289774,
    "lon": 112.738914,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289775,
    "lon": 112.738746,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289776,
    "lon": 112.738532,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289778,
    "lon": 112.738227,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289785,
    "lon": 112.737992,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289799,
    "lon": 112.737742,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289833,
    "lon": 112.737465,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.289953,
    "lon": 112.736979,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.289993,
    "lon": 112.736928,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.290041,
    "lon": 112.736845,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.290114,
    "lon": 112.736634,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.290162,
    "lon": 112.736485,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.290212,
    "lon": 112.736324,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.290386,
    "lon": 112.736407,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.290782,
    "lon": 112.736598,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.291404,
    "lon": 112.736896,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.291513,
    "lon": 112.736949,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.291865,
    "lon": 112.737118,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.292029,
    "lon": 112.737197,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.292264,
    "lon": 112.737317,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.292178,
    "lon": 112.737562,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.292013,
    "lon": 112.738107,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.291235,
    "lon": 112.737878,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.289833,
    "lon": 112.737465,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.289953,
    "lon": 112.736979,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.289993,
    "lon": 112.736928,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.290041,
    "lon": 112.736845,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.290114,
    "lon": 112.736634,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.290162,
    "lon": 112.736485,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.290212,
    "lon": 112.736324,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.290285,
    "lon": 112.736149,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.290188,
    "lon": 112.736088,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.289773,
    "lon": 112.735877,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.289641,
    "lon": 112.73581,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.289203,
    "lon": 112.735587,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.28859,
    "lon": 112.735278,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.288267,
    "lon": 112.735121,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.288153,
    "lon": 112.735066,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.288064,
    "lon": 112.735022,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.287931,
    "lon": 112.734956,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.287228,
    "lon": 112.734608,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.286463,
    "lon": 112.734222,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.286369,
    "lon": 112.734184,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.284672,
    "lon": 112.733363,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.284583,
    "lon": 112.733315,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.284827,
    "lon": 112.732761,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284843,
    "lon": 112.73273,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285023,
    "lon": 112.732363,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285315,
    "lon": 112.731771,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285349,
    "lon": 112.731718,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285404,
    "lon": 112.731675,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285469,
    "lon": 112.731655,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.285663,
    "lon": 112.731652,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.286097,
    "lon": 112.73163,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.286341,
    "lon": 112.731623,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.286443,
    "lon": 112.731613,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.286869,
    "lon": 112.731588,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.287091,
    "lon": 112.731566,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.287108,
    "lon": 112.731565,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.28757,
    "lon": 112.731597,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.28774,
    "lon": 112.731597,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.287922,
    "lon": 112.731589,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.288055,
    "lon": 112.731574,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.288232,
    "lon": 112.731546,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.288541,
    "lon": 112.731542,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.288659,
    "lon": 112.731541,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.289091,
    "lon": 112.731539,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.289469,
    "lon": 112.731526,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.28971,
    "lon": 112.731566,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.289816,
    "lon": 112.731559,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.289949,
    "lon": 112.731522,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.290021,
    "lon": 112.731495,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.290098,
    "lon": 112.731448,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.290269,
    "lon": 112.731327,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.290423,
    "lon": 112.731243,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.290546,
    "lon": 112.731348,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.290598,
    "lon": 112.731399,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.290783,
    "lon": 112.731581,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.290966,
    "lon": 112.731772,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.291421,
    "lon": 112.731451,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.291489,
    "lon": 112.731559,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.291594,
    "lon": 112.731712,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.291629,
    "lon": 112.731749,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.291674,
    "lon": 112.731764,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.291729,
    "lon": 112.731767,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.291816,
    "lon": 112.731754,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.291882,
    "lon": 112.731777,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.291928,
    "lon": 112.731815,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.291954,
    "lon": 112.731884,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.292114,
    "lon": 112.732065,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.292309,
    "lon": 112.732322,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.292569,
    "lon": 112.732124,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.292909,
    "lon": 112.731872,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293079,
    "lon": 112.731753,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293247,
    "lon": 112.731634,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293295,
    "lon": 112.731597,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293413,
    "lon": 112.731761,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293478,
    "lon": 112.73184,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293606,
    "lon": 112.732027,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293729,
    "lon": 112.7322,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.293339,
    "lon": 112.732542,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.292937,
    "lon": 112.732892,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.292531,
    "lon": 112.733256,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.292066,
    "lon": 112.733673,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.29156,
    "lon": 112.734162,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.291337,
    "lon": 112.734388,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.291123,
    "lon": 112.734617,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.290911,
    "lon": 112.734907,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.29067,
    "lon": 112.735227,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.290572,
    "lon": 112.735411,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.290252,
    "lon": 112.735967,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.290188,
    "lon": 112.736088,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.290115,
    "lon": 112.736276,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.290071,
    "lon": 112.736411,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.289985,
    "lon": 112.736675,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.289949,
    "lon": 112.736832,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.289944,
    "lon": 112.73691,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.289953,
    "lon": 112.736979,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.289833,
    "lon": 112.737465,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.289799,
    "lon": 112.737742,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.289785,
    "lon": 112.737992,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.289778,
    "lon": 112.738227,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.289776,
    "lon": 112.738532,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.289775,
    "lon": 112.738746,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.289774,
    "lon": 112.738914,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.289473,
    "lon": 112.738969,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.289289,
    "lon": 112.739,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.289128,
    "lon": 112.739031,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.288744,
    "lon": 112.739089,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.28844,
    "lon": 112.73915,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.288214,
    "lon": 112.739195,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.287816,
    "lon": 112.739261,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.287518,
    "lon": 112.739314,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.28733,
    "lon": 112.73935,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.287197,
    "lon": 112.739375,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.28697,
    "lon": 112.739412,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.286231,
    "lon": 112.739561,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.286067,
    "lon": 112.739589,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.285552,
    "lon": 112.739676,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.284806,
    "lon": 112.73982,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284736,
    "lon": 112.739833,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284666,
    "lon": 112.739847,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284588,
    "lon": 112.739863,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284551,
    "lon": 112.739871,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.284222,
    "lon": 112.739932,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.283941,
    "lon": 112.739984,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.282869,
    "lon": 112.740183,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.282862,
    "lon": 112.740095,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.282878,
    "lon": 112.740009,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.28305,
    "lon": 112.739599,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.283324,
    "lon": 112.738966,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.283075,
    "lon": 112.738844,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.282314,
    "lon": 112.738472,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.282544,
    "lon": 112.737963,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.28274,
    "lon": 112.737527,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.282597,
    "lon": 112.737455,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.282394,
    "lon": 112.737895,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.282156,
    "lon": 112.738426,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.281835,
    "lon": 112.739148,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.281331,
    "lon": 112.740293,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.281266,
    "lon": 112.740478,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.281078,
    "lon": 112.740519,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.280883,
    "lon": 112.740558,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.280439,
    "lon": 112.74064,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.28037,
    "lon": 112.740652,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.280294,
    "lon": 112.740666,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.27989,
    "lon": 112.740738,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.279764,
    "lon": 112.74076,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.279354,
    "lon": 112.740837,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.278613,
    "lon": 112.74098,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.278093,
    "lon": 112.741083,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.277934,
    "lon": 112.741112,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.277545,
    "lon": 112.741195,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.277379,
    "lon": 112.741231,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.277312,
    "lon": 112.741249,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.276974,
    "lon": 112.741337,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.276694,
    "lon": 112.741397,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.276451,
    "lon": 112.741442,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.276281,
    "lon": 112.741473,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.275845,
    "lon": 112.741558,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.275578,
    "lon": 112.741603,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.275401,
    "lon": 112.741636,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.275123,
    "lon": 112.741691,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.274968,
    "lon": 112.741725,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.274796,
    "lon": 112.741764,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.274639,
    "lon": 112.741802,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.274591,
    "lon": 112.741816,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.274102,
    "lon": 112.741958,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.274009,
    "lon": 112.741989,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273675,
    "lon": 112.742099,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273522,
    "lon": 112.742133,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273454,
    "lon": 112.742136,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273373,
    "lon": 112.742131,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273274,
    "lon": 112.74212,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273241,
    "lon": 112.742115,
    "ele": 6,
    "time": null
  },
  {
    "lat": -7.273118,
    "lon": 112.742086,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.272966,
    "lon": 112.742049,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.27281,
    "lon": 112.741998,
    "ele": 7,
    "time": null
  },
  {
    "lat": -7.272708,
    "lon": 112.74196,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.272616,
    "lon": 112.741928,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.272513,
    "lon": 112.741887,
    "ele": 8,
    "time": null
  },
  {
    "lat": -7.27248,
    "lon": 112.741874,
    "ele": 9,
    "time": null
  },
  {
    "lat": -7.272418,
    "lon": 112.741851,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.272323,
    "lon": 112.741815,
    "ele": 10,
    "time": null
  },
  {
    "lat": -7.272211,
    "lon": 112.741771,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.272134,
    "lon": 112.741748,
    "ele": 11,
    "time": null
  },
  {
    "lat": -7.272092,
    "lon": 112.741737,
    "ele": 12,
    "time": null
  },
  {
    "lat": -7.272026,
    "lon": 112.741723,
    "ele": 13,
    "time": null
  },
  {
    "lat": -7.271759,
    "lon": 112.74168,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.271698,
    "lon": 112.741672,
    "ele": 14,
    "time": null
  },
  {
    "lat": -7.271451,
    "lon": 112.741649,
    "ele": 15,
    "time": null
  },
  {
    "lat": -7.271306,
    "lon": 112.741639,
    "ele": 16,
    "time": null
  },
  {
    "lat": -7.271088,
    "lon": 112.741623,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.271062,
    "lon": 112.741621,
    "ele": 17,
    "time": null
  },
  {
    "lat": -7.270938,
    "lon": 112.741612,
    "ele": 18,
    "time": null
  },
  {
    "lat": -7.270793,
    "lon": 112.741609,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.270643,
    "lon": 112.741606,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.270393,
    "lon": 112.7416,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.270247,
    "lon": 112.741594,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.270214,
    "lon": 112.741592,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.270172,
    "lon": 112.74159,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.270042,
    "lon": 112.741588,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.269903,
    "lon": 112.741587,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.269885,
    "lon": 112.741585,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.269825,
    "lon": 112.741585,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.269611,
    "lon": 112.741565,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.269298,
    "lon": 112.741514,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.269254,
    "lon": 112.741532,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.269184,
    "lon": 112.74152,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.269054,
    "lon": 112.741501,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.268988,
    "lon": 112.741489,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.268948,
    "lon": 112.741484,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.268897,
    "lon": 112.741477,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.268864,
    "lon": 112.741473,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.26873,
    "lon": 112.741454,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.268704,
    "lon": 112.74145,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.268424,
    "lon": 112.741408,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.267845,
    "lon": 112.74131,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.2679,
    "lon": 112.741457,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.267977,
    "lon": 112.741672,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.268061,
    "lon": 112.741905,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.268099,
    "lon": 112.74201,
    "ele": 30,
    "time": null
  },
  {
    "lat": -7.268365,
    "lon": 112.742719,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.268672,
    "lon": 112.743541,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.268828,
    "lon": 112.74397,
    "ele": 29,
    "time": null
  },
  {
    "lat": -7.269178,
    "lon": 112.743844,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.269379,
    "lon": 112.743774,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.269462,
    "lon": 112.743742,
    "ele": 28,
    "time": null
  },
  {
    "lat": -7.269907,
    "lon": 112.743566,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.269989,
    "lon": 112.743538,
    "ele": 27,
    "time": null
  },
  {
    "lat": -7.270084,
    "lon": 112.743502,
    "ele": 26,
    "time": null
  },
  {
    "lat": -7.270178,
    "lon": 112.743467,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.270297,
    "lon": 112.743449,
    "ele": 25,
    "time": null
  },
  {
    "lat": -7.270491,
    "lon": 112.743382,
    "ele": 24,
    "time": null
  },
  {
    "lat": -7.270678,
    "lon": 112.743317,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.27083,
    "lon": 112.743264,
    "ele": 23,
    "time": null
  },
  {
    "lat": -7.271033,
    "lon": 112.743189,
    "ele": 22,
    "time": null
  },
  {
    "lat": -7.271168,
    "lon": 112.743139,
    "ele": 21,
    "time": null
  },
  {
    "lat": -7.271386,
    "lon": 112.743062,
    "ele": 20,
    "time": null
  },
  {
    "lat": -7.271463,
    "lon": 112.743035,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.271512,
    "lon": 112.742979,
    "ele": 19,
    "time": null
  },
  {
    "lat": -7.271524,
    "lon": 112.742975,
    "ele": 18,
    "time": null
  }
];

/**
 * Demo route: Rute nyata Surabaya City Loop 10km (mengikuti jalan raya nyata OSRM)
 */
export function generateDemoRoute() {
  const trackPoints = REAL_ROAD_POINTS;

  const lats = trackPoints.map((p) => p.lat);
  const lons = trackPoints.map((p) => p.lon);
  const bounds = [
    [Math.min(...lats), Math.min(...lons)],
    [Math.max(...lats), Math.max(...lons)],
  ];

  const waypoints = [
    { lat: trackPoints[0].lat, lon: trackPoints[0].lon, name: "Start / Finish — Bambu Runcing", type: "start" },
    { lat: trackPoints[85].lat, lon: trackPoints[85].lon, name: "CP 1 — Indomaret Taman Bungkul", type: "store" },
    { lat: trackPoints[170].lat, lon: trackPoints[170].lon, name: "CP 2 — Water Station Diponegoro", type: "water" },
    { lat: trackPoints[255].lat, lon: trackPoints[255].lon, name: "CP 3 — Checkpoint Polisi Istimewa", type: "checkpoint" }
  ];

  return {
    name: "Rute Real Surabaya Loop 10km",
    trackPoints,
    waypoints,
    stats: {
      totalDistance: 9.71,
      totalElevGain: 65,
      totalElevLoss: 63,
      minEle: 12,
      maxEle: 32,
      pointCount: trackPoints.length,
    },
    elevationProfile: trackPoints
      .filter((_, i) => i % 4 === 0)
      .map((pt, i) => ({ distance: parseFloat((i * 0.03).toFixed(1)), ele: Math.round(pt.ele) })),
    bounds,
    startPoint: trackPoints[0],
    finishPoint: trackPoints[trackPoints.length - 1],
  };
}